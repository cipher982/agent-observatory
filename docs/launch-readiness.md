...
| A2 | In /Applications, sysext `activated enabled` | ⚠️ current host dirty | The v0.2 app is now installed in `/Applications`, matches `dist/`, and `/Applications/.../agents status` reports `overall: installed`. The remaining gap is system-extension state: old `0.1.0/1` and `0.1.0/3` entries are waiting to uninstall on reboot, and `0.1.0/4` is still `[activated waiting for user]`. v0.2 gate requires reboot/approval so exactly the current `0.2.0/5` extension is `[activated enabled]`. |
